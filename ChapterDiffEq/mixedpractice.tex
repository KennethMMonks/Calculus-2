...
Equate coefficients one degree at a time:
...
\solushun{The right-hand side $2y+x$ does not factor into a function of $y$ times a function of $x$, so the equation is not separable.
\\ }{0in}
...
\\ }{0in}
\end{enumerate}
\AnswerKeyEntry{a.)~~ The right-hand side $2y+x$ does not factor into a function of $y$ times a function of $x$ so there can be no separation of variables.\newline
b.)~~$y = -\frac{1}{4} -\frac{1}{2}x +Ce^{2x}$ \newline}
\end{exercise}
