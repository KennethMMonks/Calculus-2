...
\solushun{
    \begin{align*}
        ...
        a_{100} &= \frac{2 \cdot 100}{100+1} = \frac{200}{101}\\
        a_{1000} &= \frac{2 \cdot 1000}{1000+1} = \frac{2000}{1001}.\\
    \end{align*}
    So, it seems that $\lim_{n\to\infty}a_n = 2$.\\
}{1in}
\item If you choose $\epsilon=0.1$, what could the corresponding $N$ be?
\solushun{For $\epsilon = 0.1$, we need $N$ such that $|2-a_N|<0.1$, which some algebra shows means $a_N > 1.9$. From the above list, we see that $\frac{20}{11}\approx1.81$, but that $a_{100}=\frac{200}{101}\approx 1.98 > 1.9$. So we can take $N=100$.\\}{1in}
\item If you choose $\epsilon=0.01$, what could the corresponding $N$ be? 
\solushun{By the same process as above, we need to find $N$ so that $|2-a_N|<0.01$ or that $a_N > 1.99$. We find that $a_{1000}=\frac{2000}{1001}\approx 1.998>1.99$. So we can use $N=1000$.\\}{1in}
\item  Write an $N-\epsilon$ proof that verifies your guess above is correct.
\solushun{Let $\epsilon$ be an arbitrary real number and choose $N=\frac{2}{\epsilon}-1$. Let $n$ be a natural number such that $n>N$. We wish to show that $a_n=\frac{2n}{n+1}$ will always be less than $\epsilon$ from 2. Proceeding:
\begin{align*}
    \left|\frac{2n}{n+1}-2\right|&=\frac{2}{n+1}\\
    &<\frac{2}{N+1}\\
    &=\frac{2}{\frac{2}{\epsilon}-1+1}\\
    &=\frac{2}{\frac{2}{\epsilon}}=2\cdot\frac{\epsilon}{2}\\
    &=\epsilon.
\end{align*}
Thus, for indices $n$ that are larger than our choice of $N$, the corresponding terms in our sequence are less than $\epsilon$ away from two, as desired.\\
}{3in}
\end{itemize}
\end{exercise}
...
\solushun{Let $\epsilon$ be an arbitrary real number and choose $N=\frac{1}{9\epsilon}-\frac{1}{3}$. Let $n$ be a natural number such that $n>N$. We wish to show that $a_n=\frac{n}{3n+1}$ will always be less than $\epsilon$ from $\frac{1}{3}$. Proceeding:
\begin{align*}
    \left|\frac{n}{3n+1}-\frac{1}{3}\right|&=\frac{1}{9n+3}\\
    &<\frac{1}{9N+3}\\
    &=\frac{1}{9\left(\frac{1}{9\epsilon}-\frac{1}{3}\right)+3}\\
    &=\frac{1}{\frac{1}{\epsilon}-3+3}=\frac{1}{\frac{1}{\epsilon}}\\
    &=\epsilon.
\end{align*}
Thus, for indices $n$ that are larger than our choice of $N$, the corresponding terms in our sequence are less than $\epsilon$ away from $\frac{1}{3}$, as desired.\\
}{3in}

\item $ \lim_{n \rightarrow \infty } \sqrt{9+1/n} = 3 $ 
\solushun{Let $\epsilon$ be an arbitrary real number and choose $N=\frac{1}{\epsilon^2+6\epsilon}$. Let $n$ be a natural number such that $n>N$. We wish to show that $a_n=\sqrt{9+1/n}$ will always be less than $\epsilon$ from $3$. Proceeding:
\begin{align*}
    \left|\sqrt{9+1/n}-3\right|&<\left|\sqrt{9+1/N}-3\right|\\
    &=\left|\sqrt{9+\frac{1}{\frac{1}{\epsilon^2+6\epsilon}}}-3\right|\\
    &=\left|\sqrt{9+\epsilon^2+6\epsilon}-3\right|\\
    &=\left|\sqrt{(\epsilon+3)^2}-3\right|\\
    &=\epsilon+3-3\\
    &=\epsilon.
\end{align*}
Thus, for indices $n$ that are larger than our choice of $N$, the corresponding terms in our sequence are less than $\epsilon$ away from three, as desired.\\
}{3in}
